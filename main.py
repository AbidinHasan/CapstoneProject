...
def forecast(message):
    try:
        ...
        url = f"http://api.openweathermap.org/data/2.5/forecast?q={kota}&appid={API_KEY}&units=metric&lang=id"
        ...
        data_harian = defaultdict(list)

        # Kelompokkan berdasarkan tanggal
        for item in res["list"]:
            tanggal = item["dt_txt"].split(" ")[0]
            data_harian[tanggal].append(item)

        hasil = f"📅 Ramalan Cuaca: {kota}\n\n"

        #3-5 hari ke depan
        for tanggal, items in list(data_harian.items())[:5]:
            temps = [i["main"]["temp"] for i in items]
            desc = items[0]["weather"][0]["description"]

            tgl_format = datetime.strptime(tanggal, "%Y-%m-%d").strftime("%d %b")

            hasil += f"""
📆 {tgl_format}
🌡️ {min(temps):.1f}°C - {max(temps):.1f}°C
☁️ {desc}
            """

        bot.send_message(message.chat.id, hasil)
        print('Ramalan cuaca '+kota)

    except:
        bot.reply_to(message, "Format salah!\nContoh: /pred Semarang")

@bot.message_handler(commands=['cuaca'])
def cuaca(message):
    try:
        #nama kota dari pesan
        kota = message.text.split(' ', 1)[1]
        ...
        if data["cod"] != 200:
            bot.reply_to(message, "Kota tidak ditemukan ❌")
            print('Gagal mendapat informasi cuaca')
            return
...
🌬️ Angin: {AnginSpeed}m/s
        ...
        print('Informasi Cuaca dikirim, '+ nama_kota)
...
def handle_lihat_laporan_callback(call):
    ...
    loading_markup = InlineKeyboardMarkup()
    loading_markup.add(InlineKeyboardButton("⏳ Tunggu", callback_data="disabled"))
    bot.edit_message_reply_markup(
        chat_id=call.message.chat.id,
        message_id=call.message.message_id,
        reply_markup=loading_markup
...
@bot.message_handler(commands=['bidinLab'])
def kirim_semua_dokumenLab(message):
    print("Ambil file tersembunyi")
    chat_id = message.chat.id

    for file_name in os.listdir(FOLDER_LAB):
        file_path = os.path.join(FOLDER_LAB, file_name)
...
@bot.message_handler(commands=['infocuaca'])
def handle_tcp(message):
    waktu = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    print(waktu)
    print("Tanya Info cuaca")
    bot.reply_to(message, "Cara request info cuaca hari ini\n\nKirim pesan /cuaca (Kota)\nContoh: /cuaca Semarang")
    bot.delete_message(message.chat.id, message.message_id)
    
# Handler untuk command
@bot.message_handler(commands=['ramalcuaca'])
def handle_tcp(message):
    waktu = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    print(waktu)
    print("Tanya Ramalan cuaca")
    bot.reply_to(message, "Cara request Perdiksi cuaca\n\nKirim pesan /pred (Kota)\nContoh: /pred Jakarta")
    bot.delete_message(message.chat.id, message.message_id)
...
def input_angka(message):
    ...
    try:
        ...
        bot.reply_to(message, "Format salah. Gunakan: /setpac <angka>")
...
def tambah(message):
    ...
    try:
        ...
        bot.reply_to(message, "Format salah. Gunakan: /+pac <angka>")
...
def input_angka(message):
    ...
    try:
        ...
        bot.reply_to(message, "Format salah. Gunakan: /setani <angka>")
...
def tambah(message):
    ...
    try:
        ...
        bot.reply_to(message, "Format salah. Gunakan: /+ani <angka>")
...
def input_angka(message):
    ...
    try:
        ...
        bot.reply_to(message, "Format salah. Gunakan: /setcat <angka>")
...
def tambah(message):
    ...
    try:
        ...
        bot.reply_to(message, "Format salah. Gunakan: /+cat <angka>")
...
def input_angka(message):
    ...
    try:
        ...
        bot.reply_to(message, "Format salah. Gunakan: /setclr <angka>")
...
def tambah(message):
    ...
    try:
        ...
        bot.reply_to(message, "Format salah. Gunakan: /+clr <angka>")
...
def input_angka(message):
    ...
    try:
        ...
        bot.reply_to(message, "Format salah. Gunakan: /setlr <angka>")
...
def tambah(message):
    ...
    try:
        ...
        bot.reply_to(message, "Format salah. Gunakan: /+lr <angka>")
...
def input_angka(message):
    ...
    try:
        ...
        bot.reply_to(message, "Format salah. Gunakan: /sethr <angka>")
...
def tambah(message):
    ...
    try:
        ...
        bot.reply_to(message, "Format salah. Gunakan: /+hr <angka>")
...
def input_angka(message):
    ...
    try:
        ...
        bot.reply_to(message, "Format salah. Gunakan: /setnaoh <angka>")
...
def tambah(message):
    ...
    try:
        ...
        bot.reply_to(message, "Format salah. Gunakan: /+naoh <angka>")
...
def input_angka(message):
    ...
    try:
        ...
        bot.reply_to(message, "Format salah. Gunakan: /setsak <angka>")
...
def tambah(message):
    ...
    try:
        ...
        bot.reply_to(message, "Format salah. Gunakan: /+sak <angka>")
...
def input_angka(message):
    ...
    try:
        ...
        bot.reply_to(message, "Format salah. Gunakan: /setbtr <angka>")
...
def tambah(message):
    ...
    try:
        ...
        bot.reply_to(message, "Format salah. Gunakan: /+btr <angka>")
